...
class HIsomap(BaseEstimator):
    ...
    def fit_transform(self, X, y=None, init=None):
        ...
        dist_matrix, lens, bp = mapper.fit_transform(
            X, projection=self.filter_function)
        ...
        if self.show_skeleton == "on":
            self._plot_data_with_skeleton(
                X, links, Landmark, lens.flatten(), bp)

        Index_of_landmarks = []
        for i in range(0, len(Landmark)):
            Index = [x for x, y in enumerate(
                X) if y[0] == Landmark[i, 0] and y[1] == Landmark[i, 1] and y[2] == Landmark[i, 2]]
